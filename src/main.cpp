...
void loop()
{
  ...
  if (!MQTTclient.connected())
  {
    ...
    {
      MQTTclient.connect("Smartmeter");
      delay(1000);
    }
  }

  MQTTclient.publish("Smartmeter/IP Adresse", WiFi.localIP().toString().c_str());

  delay(5000);
  Serial.println("waiting 5 second...");
  TelnetStream.println("waiting 5 second...");
}